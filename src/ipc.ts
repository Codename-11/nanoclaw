        ...
        if (fs.existsSync(messagesDir)) {
          ...
          for (const file of messageFiles) {
            ...
            try {
              ...
              if (isMain) {
                if (
                  data.type === 'discord_delete_message' &&
                  data.channelId &&
                  data.messageId &&
                  deps.deleteMessage
                ) {
                  ...
                  logger.info(
                    ...
                    'IPC discord message deleted',
                  );
                } else if (
                  data.type === 'discord_delete_messages' &&
                  data.channelId &&
                  data.count &&
                  deps.deleteMessages
                ) {
                  const deleted = await deps.deleteMessages(
                    data.channelId,
                    data.count,
                  );
                  logger.info(
                  ...
                  fs.writeFileSync(
                    resultFile,
                    JSON.stringify({
                      type: 'discord_delete_messages_result',
                      deleted,
                    }),
                  );
                } else if (
                  data.type === 'discord_create_channel' &&
                  data.channelName &&
                  deps.createChannel
                ) {
                  const result = await deps.createChannel(
                    data.channelName,
                    data.channelType || 'text',
                    data.topic,
                  );
                  logger.info(
                    { ...result, sourceGroup },
                    'IPC discord channel created',
                  );
                  const resultFile = path.join(
                    ipcBaseDir,
                    sourceGroup,
                  ...
                  fs.writeFileSync(
                    resultFile,
                    JSON.stringify({
                      type: 'discord_create_channel_result',
                      ...result,
                    }),
                  );
                } else if (
                  data.type === 'discord_edit_channel' &&
                  data.channelId &&
                  deps.editChannel
                ) {
                  await deps.editChannel(data.channelId, {
                    name: data.channelName,
                    topic: data.topic,
                  });
                  logger.info(
                    { channelId: data.channelId, sourceGroup },
                    'IPC discord channel edited',
                  );
                } else if (
                  data.type === 'discord_get_members' &&
                  deps.getMembers
                ) {
                  const members = await deps.getMembers();
                  logger.info(
                    { count: members.length, sourceGroup },
                    'IPC discord members fetched',
                  );
                  const resultFile = path.join(
                    ipcBaseDir,
                    sourceGroup,
                  ...
                  fs.writeFileSync(
                    resultFile,
                    JSON.stringify({
                      type: 'discord_get_members_result',
                      members,
                    }),
                  );
                }
