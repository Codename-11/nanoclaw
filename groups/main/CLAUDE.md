...
- **Self-build**: `mcp__nanoclaw__self_build` — spawn **Mini-Daemon** (🔧), an interactive Claude Code sidecar on the host that modifies your own codebase. Mini-Daemon streams progress to Discord and users can talk to it. Includes spawn validation, heartbeat monitoring, result embeds (green/red/yellow), and status tracking. Changes are validated (build + test) and auto-rolled back on failure. **Merge failure protection**: if build passes but merge fails, the worktree and branch are preserved (not cleaned up) with a yellow embed — user can reply "cleanup" to discard or merge manually. **Smart reload**: after merge, only restarts if compiled source (`src/`), dependencies, or container image files changed — workspace files, docs, and agent-runner changes skip the restart. Use this when asked to add features, fix bugs, or enhance yourself.
